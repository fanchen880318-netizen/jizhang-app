...
class _AddBillSheetState extends State<AddBillSheet> {
  ...
  void _save() {
    ...
    if (amt == null || amt <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('请输入有效金额'), backgroundColor: Color(0xFFC1121F)));
      return;
    }
    if (_selectedCategory == null) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('请选择用途'), backgroundColor: Color(0xFFC1121F)));
      return;
    }
      ...
      child: Padding(
        ...
        child: SingleChildScrollView(
          child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
            // 顶部拖拽条
            Center(child: Container(width: 40, height: 4, decoration: BoxDecoration(color: const Color(0xFFC4C2BB), borderRadius: BorderRadius.circular(2)))),
            const SizedBox(height: 20),
            // 标题
            Row(children: [
              Text(_editing ? '编辑账单' : '记一笔', style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold, color: Color(0xFF211E21))),
              const Spacer(),
            ]),
            const SizedBox(height: 24),

            // 金额输入
            const Text('金额', style: TextStyle(fontSize: 13, color: Color(0xFF7B7774), fontWeight: FontWeight.w500)),
            ...
            Container(
              ...
              child: TextField(
                ...
                autofocus: !_editing,
                style: const TextStyle(fontSize: 28, fontWeight: FontWeight.w700, color: Color(0xFF5B2444)),
                decoration: InputDecoration(
                  prefixIcon: const Padding(padding: EdgeInsets.only(left: 16, bottom: 8), child: Text('¥', style: TextStyle(fontSize: 28, fontWeight: FontWeight.w700, color: Color(0xFF5B2444)))),
            ...
            // 用途选择
            const Text('用途', style: TextStyle(fontSize: 13, color: Color(0xFF7B7774), fontWeight: FontWeight.w500)),
                ...
                ...provider.categories.map((cat) {
                  ...
                  return GestureDetector(
                    ...
                    child: AnimatedContainer(
                      ...
                      decoration: BoxDecoration(
                        color: sel ? const Color(0xFF5B2444) : Colors.white,
                        borderRadius: BorderRadius.circular(22),
                        boxShadow: sel
                            ? [BoxShadow(color: const Color(0xFF5B2444).withAlpha(50), blurRadius: 8, offset: const Offset(0, 3))]
                            : [BoxShadow(color: Colors.black.withAlpha(8), blurRadius: 4, offset: const Offset(0, 1))],
                      ),
                      child: Text(cat.name, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w500, color: sel ? Colors.white : const Color(0xFF7B7774))),
                    ),
                  );
                }),
              ]),
            ...
            Row(children: [
              ...
              Container(
                decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF5B2444), Color(0xFF3D1529)]), borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: const Color(0xFF5B2444).withAlpha(40), blurRadius: 8, offset: const Offset(0, 3))]),
                child: Material(
            ...
            // 日期
            const Text('日期', style: TextStyle(fontSize: 13, color: Color(0xFF7B7774), fontWeight: FontWeight.w500)),
            const SizedBox(height: 8),
            GestureDetector(
              onTap: _pickDate,
              child: Container(
                ...
                decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withAlpha(8), blurRadius: 6, offset: const Offset(0, 2))]),
                child: Row(children: [
                  const Icon(Icons.calendar_today, size: 18, color: Color(0xFF5B2444)),
                  const SizedBox(width: 10),
                  Text(DateFormat('yyyy年M月d日').format(_selectedDate), style: const TextStyle(fontSize: 15, color: Color(0xFF211E21))),
                ]),
              ),
            ),
            const SizedBox(height: 24),
            ...
            // 备注
            const Text('备注', style: TextStyle(fontSize: 13, color: Color(0xFF7B7774), fontWeight: FontWeight.w500)),
            const SizedBox(height: 8),
            Container(
              decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(14), boxShadow: [BoxShadow(color: Colors.black.withAlpha(8), blurRadius: 6, offset: const Offset(0, 2))]),
              child: TextField(
