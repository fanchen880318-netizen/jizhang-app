...
class _HomePageState extends State<HomePage> {
    ...
    return Container(
      ...
      child: Column(children: [
              ...
              child: Row(mainAxisSize: MainAxisSize.min, children: [
                ...
                Text(
                  provider.dateRange != null
                      ? '${DateFormat('M.d').format(provider.dateRange!.start)} - ${DateFormat('M.d').format(provider.dateRange!.end)}'
                      : '全部日期',
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, crossAxisAlignment: CrossAxisAlignment.end, children: [
          ...
          Text(
            _showAmount ? '¥${NumberFormat('#,###.00').format(provider.totalAmount)}' : '¥****',
            style: const TextStyle(fontSize: 36, fontWeight: FontWeight.w700, color: Colors.white, letterSpacing: 2),
          ),
        ]),
    ...
    if (provider.bills.isEmpty) {
      return Center(
        child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Container(
            width: 80, height: 80,
            decoration: BoxDecoration(color: const Color(0xFF5B2444).withAlpha(20), borderRadius: BorderRadius.circular(40)),
            child: const Icon(Icons.receipt_long, size: 40, color: Color(0xFF5B2444)),
          ),
          ...
          Row(mainAxisAlignment: MainAxisAlignment.center, children: [
            ...
            Container(
              width: 28, height: 28,
              decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF5B2444), Color(0xFF3D1529)]), borderRadius: BorderRadius.circular(14)),
              ...
              child: Padding(
                ...
                child: Row(children: [
                  Container(
                    ...
                    child: Icon(_catIcon(bill.category), size: 22, color: color),
                  ),
                  const SizedBox(width: 12),
                  Expanded(
                    child: Column(crossAxisAlignment: CrossAxisAlignment.start, mainAxisAlignment: MainAxisAlignment.center, children: [
                      ...
                      if (bill.note.isNotEmpty)
                        Padding(padding: const EdgeInsets.only(top: 3), child: Text(bill.note, style: const TextStyle(fontSize: 13, color: Color(0xFF7B7774)), overflow: TextOverflow.ellipsis)),
                    ]),
                  ),
                  Text(
                    _showAmount ? '¥${NumberFormat('#,###.00').format(bill.amount)}' : '¥****',
                    style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: Color(0xFF5B2444)),
                  ),
                ]),
              ),
            ),
  ...
  IconData _catIcon(String name) {
    const map = {
      '网采': Icons.shopping_cart_outlined,
      '还款': Icons.credit_card_outlined,
      '地采': Icons.storefront_outlined,
...
class ExportPage extends StatelessWidget {
        ...
        child: Column(children: [
          Container(
            ...
            child: Column(children: [
              Container(
                width: 64, height: 64,
                decoration: BoxDecoration(gradient: const LinearGradient(colors: [Color(0xFF2D6A4F), Color(0xFF40916C)]), borderRadius: BorderRadius.circular(18)),
                child: const Icon(Icons.table_chart, size: 32, color: Colors.white),
              ),
              const SizedBox(height: 16),
              Text(provider.dateRange != null ? '${DateFormat('yyyy-MM-dd').format(provider.dateRange!.start)} ~ ${DateFormat('yyyy-MM-dd').format(provider.dateRange!.end)}' : '全部日期', style: const TextStyle(fontSize: 16, color: Color(0xFF7B7774))),
              const SizedBox(height: 8),
              Text('共 ${provider.bills.length} 笔', style: const TextStyle(fontSize: 14, color: Color(0xFF7B7774))),
              const SizedBox(height: 8),
              Text('¥${NumberFormat('#,###.00').format(provider.totalAmount)}', style: const TextStyle(fontSize: 32, fontWeight: FontWeight.bold, color: Color(0xFF211E21))),
            ]),
          ),
  ...
  Future<void> _export(BuildContext context, BillProvider provider) async {
    try {
      ...
      final bills = await provider.getExportBills();
      final total = await provider.getExportTotal();
      if (context.mounted) Navigator.pop(context);
      await ExcelService.exportAndShare(bills, total, '账单_${DateFormat('yyyyMMdd').format(DateTime.now())}');
    } catch (e) {
